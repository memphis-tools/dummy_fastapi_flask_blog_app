    ...
    if user:
        if current_user.id == user.id or current_user.username == "admin":
            ...
            if user_updated.email is not None:
                existing_email = (
                    session.query(models.User)
                    .filter_by(email=str(user_updated.email).lower())
                    .first()
                )
                if existing_email:
                    raise HTTPException(
            ...
            if user_updated.role is not None:
                if not database_crud_commands.is_user_role_valid(
                    str(user_updated.role).lower()
                ):
                    raise HTTPException(
                        status_code=status.HTTP_404_NOT_FOUND, detail="Role inconnu."
                    )
                if (
                    str(user_updated.role).lower() == "admin"
            ...
            if (
                str(user_updated.disabled) == "True"
                and not current_user.role == "admin"
            ):
                log_events.log_event(
...
async def update_user_password(
    ...
    if user:
        if current_user.id == user.id or current_user.username == "admin":
            if handle_passwords.check_password_input(
